 ...
 * Kept structural so the node only needs n8n-workflow at dev time.
 */
...
const SHARED_OPTIONS: INodeProperties[] = [
	{
	...
	},
	{
		...
		options: [
			...
			},
			{
				...
				value: 'openai',
				description:
					'POST {baseUrl}/chat/completions. Works for GPT, Grok, DeepSeek, GLM, Kimi, MiniMax, Gemini and most Claude models behind the gateway.',
			},
			{
				name: 'Anthropic Messages',
				value: 'anthropic',
				description:
					'{gateway}/v1/messages with x-api-key headers. For Claude/Qwen models needing native semantics.',
			},
		],
...
function modelListProperties(credentialName: string): INodeProperties {
	return {
		displayName: 'Model',
		name: 'model',
		type: 'options',
		...
		default: '',
		description:
			'The model ID. List is fetched live from the gateway. Free models are marked "-free".',
	};
}

export function makeOpenCodeChatModel(
	cfg: GatewayConfig,
): new () => INodeType {
		...
		async supplyData(this: ISupplyDataContext, itemIndex: number): Promise<SupplyData> {
			...
			if (protocol === 'anthropic') {
				...
				model = new ChatAnthropic({
					...
					topP:
						options.topP !== undefined && options.topP !== 1
							? options.topP
							: undefined,
